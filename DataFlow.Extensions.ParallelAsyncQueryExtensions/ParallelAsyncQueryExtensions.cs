...
namespace DataFlow.Extensions
{
    ...
    public static class ParallelAsyncQueryExtensions
    {
        ...
        /// <summary>
        /// **FIXED**: Creates a string from a parallel sequence in a thread-safe manner.
        ...
        public static async Task<List<T>> ToList<T>(this ParallelAsyncQuery<T> source)
        {
            ...
            // Intelligently choose the implementation based on the query's configuration.
            if (source.Settings.PreserveOrder)
            {
                // ORDERED PATH: Slower, but respects the user's explicit request for order.
                // Uses a ConcurrentDictionary to store items by their original index.
                var concurrentDict = new ConcurrentDictionary<int, T>();

                // The indexed ForEach is crucial here to get the original position.
                await source
                    .ForEach((item, index) => concurrentDict[index] = item)
                    .Do();

                // Reconstruct the ordered list by sorting by the captured index.
                return concurrentDict.OrderBy(kvp => kvp.Key).Select(kvp => kvp.Value).ToList();
            }
            else
            {
                // UNORDERED PATH: Faster. Correct for when order doesn't matter.
                // ConcurrentBag is highly optimized for this "collect all" scenario.
        ...
        /// <summary>
        /// **FIXED**: Applies an accumulator function over a sequence. This implementation is sequential
    ...
    public static class Spy_ParallelAsyncQueryExtension
    {
        ...
        /// <summary>
        /// **FIXED**: Spies on a sequence, ensuring correct pass-through behavior and thread-safe console output.
