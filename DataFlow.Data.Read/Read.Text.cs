...
public static partial class Read
{
    ...
    public static async IAsyncEnumerable<string> Text(Stream stream, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        using var file = new StreamReader(stream, leaveOpen: true);
        while (!file.EndOfStream)
        {
            cancellationToken.ThrowIfCancellationRequested();
            yield return await file.ReadLineAsync();
