# DataFlow.Core.Materialization

**A lightweight, high-performance library for materializing objects from structured data (CSV rows, test fixtures, etc.).**

[![NuGet](https://img.shields.io/nuget/v/DataFlow.Core.Materialization.svg)](https://www.nuget.org/packages/DataFlow.Core.Materialization/)
...
**After (With DataFlow.Core.Materialization):**
```csharp
[Fact]
public void ProcessOrder_ValidData_Success()
{
...
```bash
dotnet add package DataFlow.Core.Materialization
```
...
| **DataFlow.Core.Materialization** | 12 ns | **0.01x** | 0 B |
...
### ✅ Completed (v1.0)
- [x] Expression tree compilation
- [x] Schema/Order/Constructor mapping
- [x] Culture-aware parsing
- [x] Thread-safe caching
- [x] `IHasSchema` interface
- [x] Comprehensive documentation

### 🚧 Short-Term (v1.1-1.2)
- [ ] **Embedded resource support** (load CSV from assembly resources)
- [ ] **Benchmark suite** (publish official numbers)
- [ ] **Example test project** (real-world usage patterns)
- [ ] **XML documentation** (IntelliSense support)

### 🔮 Long-Term (v2.0+)
- [ ] **Roslyn source generator** (compile-time plan generation)
- [ ] **Custom converter registration** (user-defined type mappings)
- [ ] **Cache eviction policy** (LRU for long-running apps)
- [ ] **Async materialization** (for I/O-bound scenarios)
- [ ] **Validation hooks** (pre/post-materialization callbacks)
...
- **Issues:** [GitHub Issues](https://github.com/yourorg/DataFlow.Core.Materialization/issues)
- **Discussions:** [GitHub Discussions](https://github.com/yourorg/DataFlow.Core.Materialization/discussions)
