...
using static DataFlow.Framework.RegexsExtensions;
...
public class RegexTokenizer
{
    ...
    private static IEnumerable<(string groupName, (int startIndex, int Length))> Slices(IEnumerable<(string groupName, Capture capture)> capturesSortedByIndex, int MaxIndex)
    {
        int parserIndex = 0;
    ...
    private  IEnumerable<(string groupName, (int startIndex, int Length)slice )> Slices(string line)
    {
    ...
    public IEnumerable<(string groupName, string subpart)> Tokenize(string line)
    {
        return Slices(line)
            .Cases(
    ...
    // 
    private IEnumerable<(string groupName, Capture capture)> getCapturesSortedByIndex(IEnumerable<Group> result)
    {
...
public static class RegexsExtensions
{        

    public static IEnumerable<(string groupName, string subpart)> Tokenize(this string line, RegexTokenizer regxs)
    => regxs.Tokenize(line);
    ...
    private static StringBuilder Build(this StringBuilder builder, string line, IEnumerable<(string groupName, (int startIndex, int Length) slice)> slices, RgxRequests transformations )
    {
        slices.ForEach(x =>
        {
            ...
            {
                if (x.groupName == RegexTokenizer.UNMATCHED.LINE) builder.Append(map(line));
                else builder.Append(map(line.Substring(x.slice.startIndex, x.slice.Length)));
            }
            else 
            {
                if (x.groupName == RegexTokenizer.UNMATCHED.LINE) builder.Append(line);
                else builder.Append(line.AsSpan().Slice(x.slice.startIndex, x.slice.Length));
            }
        })
        .Do();
        return builder;
